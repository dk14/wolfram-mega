...
const generateCetTransaction = async (params, vout = 0) => {
    ...
    if (params.oraclePub2 === undefined) {
        ...
        return (await tx.genAliceCet(multiIn, params.alicePub, params.bobPub, twistedPk, params.aliceAmount, params.bobAmount, params.txfee, params.session, params.stateAmount, params.utxoPartyFee))?.hex;
    }
    else {
        ...
        return (await tx.genAliceCetQuorum(multiIn, params.alicePub, params.bobPub, twistedPk1, twistedPk2, twistedPk3, params.aliceAmount, params.bobAmount, params.txfee, params.session, params.stateAmount, params.utxoPartyFee))?.hex;
    }
};
...
const generateCetRedemptionTransaction = async (params, quorumno = 1) => {
    ...
    if (params.oraclePub2 === undefined) {
        const twistedPk = schnorr.adaptorPublic(params.oraclePub, params.answer, params.rValue).padStart(64, "0");
        return (await tx.genAliceCetRedemption(cetOut, twistedPk, params.alicePub, params.oracleSignature, params.amount, params.txfee, params.session, params.bobPub, params.utxoPartyFee)).hex;
    }
    else {
        const twistedPk1 = schnorr.adaptorPublic(params.oraclePub, params.answer, params.rValue).padStart(64, "0");
        const twistedPk2 = schnorr.adaptorPublic(params.oraclePub2, params.answer2 ?? params.answer, params.rValue2).padStart(64, "0");
        const twistedPk3 = schnorr.adaptorPublic(params.oraclePub3, params.answer3 ?? params.answer, params.rValue3).padStart(64, "0");
        return (await tx.genAliceCetRedemptionQuorum(quorumno, cetOut, twistedPk1, twistedPk2, twistedPk3, params.alicePub, params.bobPub, params.oracleSignature, params.oracleSignature2, params.oracleSignature3, params.amount, params.txfee, params.session, params.utxoPartyFee))?.hex;
    }
};
...
const generateDlcContract = async (params) => {
    ...
    const openingTx = await (0, exports.generateOpeningTransaction)({ ...params, alicePub: params.alicePub, bobPub: params.bobPub });
    if (!openingTx) {
