...
```
> there is no common sense reason to let a particular third-party (e.g. government) authorize your deal, unless carol takes part in the contract herself (locking conditions within multi-party contract, which itself likely come from miscalculation too). 
