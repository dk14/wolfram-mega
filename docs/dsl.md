...
##### Edge-cases
`unsafe.numeric` and `unsafe.set` re-use same observation twice in order to adapt intuitive semantics of a set to semantics of binary choice. It can be memoized, but optimality for `unsafe` is not guaranteed anyways.
