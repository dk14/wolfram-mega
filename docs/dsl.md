...
##### Edge-cases
`numeric` and `set` re-use same observation twice in order to adapt intuitive semantics of a set to semantics of binary choice. It can be memoized in enumerator, but optimality for execution of advanced features is not guaranteed anyways, and extra transactions would have to be generated regardless of such optimization.
