...
##### Edge-cases
`numeric` and `set` re-use same observation twice (extra leaf in tree) in order to adapt intuitive semantics of a set to semantics of binary choice. It is memoized as an exception.

> Turning off binary `PerfectHedge` check would allow to remove extra step. This however would reduce "edge-case" safety. It is better to defer such optimization to contract interpreter, since it already has to collapse binary trees anyways.
