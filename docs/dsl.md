...
> This restriction only applies to BTC-to-asset loan. Any other loan, e.g. asset-to-asset would rely on overlay blockchain (in OP_RETURN), thus advanced spending rules can be implemented (by issuers of corresponding assets) including OP_ANYPREVOUT without need to change anything about BTC protocol.
